...
public class AscendingOrder2 {
    ...
    private static void merge(int[] input, int start, int midpoint, int end) {
        ...
        // input[mid-1] < input[mid] -> condition for stopping merge recursion, already ordered
        ...
        // condition: when index i finish traversing left side or j right side, it doesn't matter
        //stable <=
        while(i < midpoint && j < end)
            // {35,-15} -> tmpArray={15} and 35 is missing but still in input
        ...
        // i contains always the biggest value missed, if it finishes traverse, so  it's out of while cond.
        // it means that its value it is equal to midpoint and (midpoint-i)=0, so it won't use arrayCopy
